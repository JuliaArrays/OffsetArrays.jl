...
for Z in [:ZeroBasedRange, :ZeroBasedUnitRange]
    ...
    @eval OffsetArraysCore.no_offset_view(A::$Z) = A.a
...
for Z in [:ZeroBasedRange, :ZeroBasedUnitRange]
    for R in [:AbstractRange, :AbstractUnitRange, :StepRange]
        @eval @inline function Base.getindex(A::$Z, r::$R{<:Integer})
            ...
            OffsetArraysCore._indexedby(A.a[r .+ 1], axes(r))
        end
    end
    ...
    for R in [:ZeroBasedUnitRange, :ZeroBasedRange]
        @eval @inline function Base.getindex(A::$Z, r::$R{<:Integer})
            @boundscheck checkbounds(A, r)
            OffsetArraysCore._indexedby(A.a[r.a .+ 1], axes(r))
        end
    end

    for R in [:IdOffsetRange]
    ...
    for R in [:AbstractUnitRange, :IdOffsetRange, :StepRange, :StepRangeLen, :LinRange]
        @eval @inline function Base.getindex(A::$R, r::$Z)
            @boundscheck checkbounds(A, r)
            OffsetArraysCore._indexedby(A[r.a], axes(r))
        end
    end
