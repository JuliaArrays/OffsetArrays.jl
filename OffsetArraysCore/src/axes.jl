...
```jldoctest ior
julia> using OffsetArraysCore: IdOffsetRange
...
the indices if required by the specified types. For example,
    ...
    r = OffsetArraysCore.IdOffsetRange{Int,UnitRange{Int}}(3:4)

has `r[1] == 3` and `r[2] == 4`, whereas

    r = OffsetArraysCore.IdOffsetRange{Int,Base.OneTo{Int}}(3:4)
...
!!! warning
    ...
    the indices, throwing an error when this is not achievable. For instance,
        ...
        r = convert(OffsetArraysCore.IdOffsetRange{Int,UnitRange{Int}}, 3:4)

    has `r[1] == 3` and `r[2] == 4` and would satisfy `r == 3:4`, whereas

    ```julia
    julia> convert(OffsetArraysCore.IdOffsetRange{Int,Base.OneTo{Int}}, 3:4)    # future behavior, not present behavior
...
See https://github.com/JuliaArrays/OffsetArraysCore.jl/issues/204
=#
...
# See https://github.com/JuliaArrays/OffsetArraysCore.jl/issues/214
...
See https://github.com/JuliaArrays/OffsetArraysCore.jl/pull/224#discussion_r595635143
