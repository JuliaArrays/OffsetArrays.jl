const OffsetAxis{T<:Integer} = Union{IdOffsetRange{T}, IdentityUnitRange{<:IdOffsetRange{T}}}
@static if isdefined(Base, :AbstractOneTo)
    const IntegerOrOneTo = Union{Integer, Base.AbstractOneTo{<:Integer}}
else
    const IntegerOrOneTo = Union{Integer, Base.OneTo}
end
const IntegerOrAbsUnitRange = Union{Integer, AbstractUnitRange{<:Integer}}
const IntegerOrOneToOrOffsetAxis{T<:Integer} = Union{IntegerOrOneTo, OffsetAxis{T}}
const IntegerOrOneToOrOffsetAxisOrColon{T<:Integer} = Union{IntegerOrOneToOrOffsetAxis{T}, Colon}
...
to_shape(shape) = map(_indexlength, shape)
to_shape(shape::Tuple{Vararg{IntegerOrOneTo}}) = shape
...
"""
    OffsetArraysCore.AxisConversionStyle(typeof(indices))
...
julia> OffsetArraysCore.AxisConversionStyle(::Type{NTupleOfUnitRanges{N}}) where {N} = OffsetArraysCore.TupleOfRanges();
...
_indexedby(A::AbstractArray{<:Any,0}, ::Tuple{}) = no_offset_view(A)
_indexedby(A::AbstractVector, shape::Tuple{IntegerOrOneTo}) = _indexedby(A, shape[1])
_indexedby(A::AbstractArray{<:Any,N}, ::NTuple{N,IntegerOrOneTo}) where {N} = no_offset_view(A)
_indexedby(A::AbstractVector, shape::Tuple{IntegerOrAbsUnitRange}) = _indexedby(A, shape[1])
_indexedby(A::AbstractArray{<:Any,N}, shape::NTuple{N,IntegerOrAbsUnitRange}) where {N} = OffsetArray(A, map(_offset, axes(A), shape))
